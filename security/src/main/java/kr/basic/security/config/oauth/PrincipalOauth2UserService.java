...
public class PrincipalOauth2UserService extends DefaultOAuth2UserService {
    ...
    private final BCryptPasswordEncoder encoderPw;


    // user request 는 구글ㄹ에서 코드를 받아서 accessToken을 응답 받는 객체
    @Override
    public OAuth2User loadUser(OAuth2UserRequest userRequest) throws OAuth2AuthenticationException {
        System.out.println(" user request clientRegistration : " + userRequest.getClientRegistration());
        System.out.println(" user request getAccessToke : " + userRequest.getAccessToken().getTokenValue());
